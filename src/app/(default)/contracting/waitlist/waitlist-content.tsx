...
import AddWaitlistButton from "./add-waitlist";
import AppLoading from "@/components/app-loading";
import DateSelect from "@/components/dropdowns/date-select";
import { SearchBar } from "@/components/search-bar";
import WaitlistExportButton from "./waitlist-client";
import DeleteButton from "@/components/delete-button";
import ErrorDisplay from "@/components/error-display";
import PaginationClassic from "@/components/pagination-classic";
import { useGetWaitlistQuery } from "@/redux/slices/api/waitlist-slice";
...
export default function WaitlistContent() {
  ...
  const { data, isFetching, refetch } = useGetWaitlistQuery(query, { refetchOnMountOrArgChange: true });
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-[96rem] mx-auto">
      ...
      <div className="sm:flex sm:justify-between sm:items-center mb-8">
        ...
        <div className="mb-4 sm:mb-0">
          <h1 className="text-2xl md:text-3xl text-slate-800 dark:text-slate-100 font-bold">Waitlist ✨</h1>
        </div>
        ...
        <div className="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
          ...
          <AddWaitlistButton refetch={refetch} />
        </div>
      </div>
      ...
      {hasContent && <WaitlistTable waitlist={data?.data.items!} total={data.data.meta.total} refetch={refetch} />}
