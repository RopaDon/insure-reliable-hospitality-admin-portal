...
import WaitlistOptions from "./waitlist-options";
...
export default function userTableItem({ user, refetch, onCheckboxChange, isSelected }: CustomersTableItemProps) {
    ...
    <tr>
      ...
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div className="text-xs inline-flex font-medium bg-emerald-100 dark:bg-sky-400/30 text-emerald-600 dark:text-sky-400 rounded-full text-left px-2.5 py-1">
          {user.jobExperienceLevel.name}
        </div>
      </td>
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div className="text-xs inline-flex font-medium bg-sky-100 dark:bg-sky-400/30 text-sky-600 dark:text-sky-400 rounded-full text-center px-2.5 py-1">
          {user.industry.name}
        </div>
      </td>
      ...
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
        <WaitlistOptions user={user} refetch={refetch} />
      </td>
    </tr>
  );
}
